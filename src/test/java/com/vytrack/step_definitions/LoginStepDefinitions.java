...
public class LoginStepDefinitions {
    ...
    public void user_verifies_warning_message_is_displayed(String expected) {
        Assert.assertEquals(expected, pages.loginPage().getErrorMessage());
    }

    @Then("user log in as a driver")
    public void user_log_in_as_a_driver() {
        pages.loginPage().login(ConfigurationReader.getProperty("driverusername"),
                ConfigurationReader.getProperty("driverpassword"));
    }

    @When("user log in as a {string}")
    public void user_log_in_as_a(String role) {
